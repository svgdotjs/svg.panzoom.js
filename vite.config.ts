...
const types = readFileSync('svg.panzoom.js.d.ts', 'utf8')
let emitted = false
...
const distExtras = {
  ...
  generateBundle() {
    ...
    this.emitFile({
      type: 'asset',
      fileName: 'svg.panzoom.d.mts',
      source: types
    })
    this.emitFile({
      type: 'asset',
      fileName: 'svg.panzoom.d.ts',
      source: types
    })
  }
}
  ...
  build: {
    ...
    rollupOptions: {
      ...
      output: [
        ...
        {
          format: 'es',
          entryFileNames: 'svg.panzoom.mjs',
          banner: headerLong,
          minify: false
        }
      ]
